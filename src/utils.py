...
class BotClass:
    def __init__(self):
        ...
        self.data_folder = Path.cwd().parent / "data"
...
def get_est_time(time_to_convert: datetime = None) -> str:
    ...
    readable string
    """
    desired_timezone = timezone("America/New_York")
...
def ntfs_compress_snowflake(snowflake: int) -> str:
    ntfs_blacklist = ["<", ">", ":", '"', "/", "\\", "|", "?", "*"]
    misc_blacklist = [" "]
    chars = [
        chr(i)
        for i in range(32, 127)
        if (chr(i) not in ascii_lowercase)
        and (chr(i) not in ntfs_blacklist)
        and (chr(i) not in misc_blacklist)
    ]
    map_len = len(chars)

    output = []
    while snowflake > 0:
        dividend = snowflake % map_len
        output.append(chars[dividend])
        divisor = map_len if snowflake > map_len else snowflake
        snowflake = int((snowflake - dividend) / divisor)
