...
async def SaveRoles(servers):
    ...
    for server in servers:
        ...
        with open("___roles___.txt", "w") as inputfile:
...
async def SavePMs(channel):
    msgs = []
    async for msg in client.logs_from(client.get_channel(channel), limit=500000):
        msgl = ""
        msgl = msg.timestamp.strftime("[%Y-%b-%d_%Hh%Mm%Ss] ") + msg.author.name + ": "
        try:
            msg.attachments[0]
            for i in msg.attachments:
                msgl += i["url"]
            ...
            msgl += msg.content
        msgl = msgl.encode("ascii", errors="ignore").decode("ascii", errors="ignore")
        msgl += "\n\n"
        msgs.insert(0, msgl)
    with open("pms.txt", "a") as inputfile:
        for i in msgs:
            try:
                inputfile.write(i)
            except:
...
print("--Init--")
token = "GSDFHFDGNFGHNBFDGBDFGBDFGN"
...
async def main():
    ...
    # await SaveMessagesWithIDs(servers,)
    # await SaveImages(servers)
    # await Reversefiles("ABC/chatlogs")
    await SaveMessagesWithIDs(["123456789012345678"])
...
token = "GSDFHFDGNFGHNBFDGBDFGBDFGN"  # Fool Selfbot
