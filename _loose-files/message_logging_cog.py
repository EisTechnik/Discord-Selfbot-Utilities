...
# from utils import BotClass, log_with_time, get_est_time
from discord.ext import commands

from utils import get_est_time, log_with_time
...
class MessageLogging(commands.Cog):
    def __init__(self, client: commands.Bot, guild_id: int):
        self.client = client
    ...
    async def find_channel_checkpoints(self):
        log_with_time("Identifying what channels to scrape/when to scrape from")
        time_maps = {}
        existing_channels = [
            item[0]
        ...
        for channel_id in existing_channels:
            channel_object = self.guild.get_channel(channel_id)
            if channel_object is None:
                log_with_time(f"{channel_id} not accessible")
        ...
        all_channels = self.guild.text_channels  # + self.guild.threads
    ...
    async def scrape_server_messages(
        ...
        if time_maps is None:
            log_with_time("No message log database found, scraping full server")
            channels = self.guild.text_channels  # + self.guild.threads
        else:
            log_with_time(f"Updating logs for {len(time_maps)} channels")
        ...
        for channel in channels:
            if (
                ...
                log_with_time(f"Scraping all messages in #{channel.name}")
                after_datetime = None
            ...
            try:
                ...
                log_with_time(f"Exception when processing #{channel.name} ({e})")
            if not message_rows:
                continue
            log_with_time(
        ...
        log_with_time("Message scraping complete")
        self.loading = False

        if self.message_buffer:
            log_with_time(f"Committing {len(self.message_buffer)} unprocessed messages")
            await self.insert_many_to_db(self.message_buffer)
            log_with_time(f"Committed {len(self.message_buffer)} unprocessed messages")
            self.message_buffer = []

        log_with_time("Message logging ready")
    ...
    async def message_to_db_columns(self, message: Message) -> MessageColumnsType:
